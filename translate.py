...
def respondTo(sess, model, inVocab, outVocab, query):
  # Get token-ids for the input sentence.
  token_ids = data_utils.sentence_to_token_ids(tf.compat.as_bytes(query), inVocab)
  # Which bucket does it belong to?
  bucket_id = len(_buckets) - 1
  for i, bucket in enumerate(_buckets):
    if bucket[0] >= len(token_ids):
    ...
    logging.warning("Sentence truncated: %s", query)
